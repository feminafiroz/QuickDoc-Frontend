...
const Navbar: React.FC = () => {
  ...
  return (
    ...
    {user.isAuthenticated && user.role === 'user' ? (
            <>
            <li>
              <Link to="/user/profile" className="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-green-700 md:p-0 dark:text-white md:dark:hover:text-green-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">Profile</Link>
            </li>
            <li>
      ...
      <Link to="/user/login" className="text-white px-3 py-2 text-sm font-medium bg-green-700 hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-opacity-50 rounded-md ml-2">Login</Link>
     )}
          </ul>
        </div>
      </div>
